...
// SignUp Controller (Modified: Sends OTP instead of creating account) >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
exports.forgotPassword = asyncWrapper(async (req, res, next) => {
  ...
  //when we call this metod  getResetPasswordToken  . so in userModel resetPasswordToken has reset token added and resetPasswordExpire also exprie value added but not saved to data base
  ...
  try {
    ...
    // if there any Error then  user.resetPasswordToken and user.resetPasswordExpire has value saved already then  both od them for fresh value if user want to try again
